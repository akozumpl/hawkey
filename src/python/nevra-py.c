...
nevra_init(_NevraObject *self, PyObject *args, PyObject *kwds)
{
    ...
    char *kwlist[] = {"name", "epoch", "version", "release", "arch", NULL};

    if (!PyArg_ParseTupleAndKeywords(args, kwds, "z|Ozzz", kwlist,
                    &name, &epoch_o, &version, &release, &arch))
        return -1;
...
evr(_NevraObject *self, PyObject *unused)
{
    char *str;
    PyObject *o;
    str = hy_nevra_evr(self->nevra->epoch, self->nevra->version,
        self->nevra->release);
...
evr_cmp(_NevraObject *self, PyObject *args)
{
    ...
    if (sack == NULL || nevra == NULL)
        return NULL;
    char *self_evr = hy_nevra_evr(self->nevra->epoch, self->nevra->version,
        self->nevra->release);
    char *other_evr = hy_nevra_evr(nevra->epoch, nevra->version,
        nevra->release);
