...
reldep_init(_ReldepObject *self, PyObject *args, PyObject *kwds)
{
    ...
    char *name, *evr = NULL;
    if (!PyArg_ParseTuple(args, "O!s", &sack_Type, &sack, &nevra))
	return -1;
    HySack csack = sackFromPyObject(sack);
    if (csack == NULL)
	return -1;

    if (hy_parse_nevra(nevra, &name, &evr, &cmp_type) == -1) {
        PyErr_Format(HyExc_Value, "Wrong reldep format: %s", nevra);
        return -1;
    }
