...
START_TEST(test_query_requires)
{
    ...
    q = hy_query_create(test_globals.sack);
    hy_query_filter_in(q, HY_PKG_REPO, HY_EQ, repolist);
    hy_query_filter_requires(q, HY_EQ, "oxygen", NULL);
    fail_unless(query_count_results(q) == 1);
    hy_query_free(q);

    q = hy_query_create(test_globals.sack);
    hy_query_filter_in(q, HY_PKG_REPO, HY_EQ, repolist);
    hy_query_filter_requires(q, HY_EQ|HY_LT, "oxygen", "3.0");
    fail_unless(query_count_results(q) == 1);
    hy_query_free(q);

    q = hy_query_create(test_globals.sack);
    hy_query_filter_in(q, HY_PKG_REPO, HY_EQ, repolist);
    hy_query_filter_requires(q, HY_GT, "oxygen", "1.0");
    fail_unless(query_count_results(q) == 1);
    hy_query_free(q);

    q = hy_query_create(test_globals.sack);
    hy_query_filter_in(q, HY_PKG_REPO, HY_EQ, repolist);
    hy_query_filter_requires(q, HY_NEQ, "oxygen", NULL);
    fail_unless(query_count_results(q) == TEST_EXPECT_MAIN_NSOLVABLES-1);
    hy_query_free(q);

    q = hy_query_create(test_globals.sack);
    hy_query_filter_in(q, HY_PKG_REPO, HY_EQ, repolist);
    hy_query_filter_requires(q, HY_NEQ, "oxygen", "2");
    fail_unless(query_count_results(q) == TEST_EXPECT_MAIN_NSOLVABLES-1);
    hy_query_free(q);

   q = hy_query_create(test_globals.sack);
    hy_query_filter_in(q, HY_PKG_REPO, HY_EQ, repolist);
    hy_query_filter_requires(q, HY_NEQ, "oxygen", "2.1");
